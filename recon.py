...
def bucket_3h(ts_report: pd.Series, origin: pd.Timestamp | None = None) -> pd.Series:
    """Floor timestamps into 3-hour buckets.

    If `origin` is provided, buckets are aligned to that origin (e.g., 09:01) instead of
    the default wall-clock 00:00/03:00/06:00 boundaries.
    """
    if origin is None:
        return ts_report.dt.floor("3H")
    origin = pd.Timestamp(origin)
    delta = (ts_report - origin)
    idx = (delta.dt.total_seconds() // (3 * 3600)).astype("int64")
    return origin + idx * pd.Timedelta(hours=3)
...
def reconcile_exact(
    ...
    for df in (matched, late_sync, missing_true):
        df["bucket_3h"] = bucket_3h(df["ts_report_backend"], origin=report_start)
